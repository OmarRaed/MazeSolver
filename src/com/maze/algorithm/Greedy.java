...
/**
 * An algorithm class that calculates the solution path based on Greedy first search algorithm
 */
public class Greedy {
    ...
    private List<TreeNode> leafMazes = new ArrayList<>();
     ...
     */
    public void explore(TreeNode tree) {
        ...
        if (bottomMaze.move(BOTTOM_MOVE) && !bottomMaze.isVisited(visitedMazes)) {
            ...
            TreeNode bottomMoveTree = new TreeNode(bottomMaze, tree);
        ...
        if (rightMaze.move(RIGHT_MOVE) && !rightMaze.isVisited(visitedMazes)) {
            ...
            //create a new tree node with this maze
            TreeNode rightMoveTree = new TreeNode(rightMaze, tree);
        ...
        if (leftMaze.move(LEFT_MOVE)) {
            ...
            //create a new tree node with this maze
            TreeNode leftMoveTree = new TreeNode(leftMaze, tree);
        ...
        if (topMaze.move(TOP_MOVE)) {
            ...
            //create a new tree node with this maze
            TreeNode topMoveTree = new TreeNode(topMaze, tree);
    ...
    /**
     * Method called to find a solution for a level using Greedy algorithm
     */
    public int runGreedy(String level) {
        ...
        while (!isSolved) {
            ...
            //sort the leaf list by heurstic function
            Collections.sort(leafMazes);

            //get the maze at 0 index (least heurstic function)
            TreeNode tempMaze = leafMazes.get(0);
